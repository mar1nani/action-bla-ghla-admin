...
function normalizeOrderStockStatus(status) {
  return status === "precommande" ? "precommande" : "stock_disponible";
}
...
export function buildAppState(store) {
  ...
  const catalogDaily = Array.isArray(analyticsStore.catalogDaily)
    ? analyticsStore.catalogDaily
        .map((entry) => ({
          date: formatReference(entry?.date),
  ...
  for (const order of orders) {
    ...
    const consumesMoroccoStock = normalizeOrderStockStatus(order.stockStatus) !== "precommande";
  ...
  const enrichedOrders = [...orders]
    ...
    .map((order) => {
      ...
      return {
        ...order,
        stockStatus: normalizeOrderStockStatus(order.stockStatus),
  ...
  const dashboard = {
    totals: {
      ...
      totalRevenueWithUnpaidMad: round(
        sum(enrichedOrders, (order) => order.customerTotalMad - order.deliveryPriceMad),
      ),
      totalRevenueMad: round(
        sum(
          enrichedOrders.filter((order) => order.remainingToPayMad <= 0),
          (order) => order.customerTotalMad - order.deliveryPriceMad,
        ),
      ),
      totalPurchaseSpendMad: round(
        sum(enrichedPurchases, (purchase) => purchase.totalCostEur) * Number(settings.eurToMad || 0),
      ),
      totalShippingSpendMad: round(
        sum(enrichedShipments, (shipment) => shipment.shippingPriceEur) *
          Number(settings.eurToMad || 0),
      ),
      totalSoldProductsProfitWithUnpaidMad: round(
        sum(enrichedOrders, (order) => order.totalProfitMad || 0),
      ),
      totalSoldProductsProfitWithoutUnpaidMad: round(
        sum(
          enrichedOrders.filter((order) => order.remainingToPayMad <= 0),
          (order) => order.totalProfitMad || 0,
        ),
      ),
      totalProfitMad: 0,
      totalProfitWithoutUnpaidMad: 0,
      totalApproxDifferenceWithUnpaidMad: 0,
      totalApproxDifferenceWithoutUnpaidMad: 0,
    },
  ...
  const totalCostSpendMad = round(
    (dashboard.totals.totalPurchaseSpendMad || 0) + (dashboard.totals.totalShippingSpendMad || 0),
  );
  dashboard.totals.totalProfitMad = round(
    (dashboard.totals.totalRevenueWithUnpaidMad || 0) - totalCostSpendMad,
  );
  dashboard.totals.totalProfitWithoutUnpaidMad = round(
    (dashboard.totals.totalRevenueMad || 0) - totalCostSpendMad,
  );
  dashboard.totals.totalApproxDifferenceWithUnpaidMad = round(
    totalCostSpendMad - (dashboard.totals.totalRevenueWithUnpaidMad || 0),
  );
  dashboard.totals.totalApproxDifferenceWithoutUnpaidMad = round(
    totalCostSpendMad - (dashboard.totals.totalRevenueMad || 0),
  );

  const recentCatalogDates = buildRecentDateKeys(14);
  const catalogDailyMap = new Map(catalogDaily.map((entry) => [entry.date, entry.uniqueSessions]));
  const recentCatalogTraffic = recentCatalogDates.map((dateKey) => ({
    date: dateKey,
    label: formatAnalyticsDateLabel(dateKey),
    uniqueSessions: catalogDailyMap.get(dateKey) || 0,
  }));
  const todayDateKey = recentCatalogDates.at(-1) || getParisDateKey();
  const catalogTodaySessions =
    recentCatalogTraffic.find((entry) => entry.date === todayDateKey)?.uniqueSessions || 0;
  const catalogLast7DaysSessions = recentCatalogTraffic
    .slice(-7)
    .reduce((total, entry) => total + entry.uniqueSessions, 0);

  dashboard.totals.catalogTodaySessions = catalogTodaySessions;
  dashboard.totals.catalogLast7DaysSessions = catalogLast7DaysSessions;
  ...
  const stats = {
    ...
    topCities: Object.entries(
      ...
      .slice(0, 6),
    catalogTrafficDaily: recentCatalogTraffic,
  };
  ...
  return {
    ...
    analytics: {
      catalogTraffic: {
        todaySessions: catalogTodaySessions,
        last7DaysSessions: catalogLast7DaysSessions,
        daily: recentCatalogTraffic,
      },
    },
  };
}
