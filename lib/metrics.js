...
export function buildAppState(store) {
  ...
  const enrichedProducts = products
    .map((product) => {
      ...
      const avgLandedCostEur =
        metric.shippedQty > 0
          ? round(metric.landedSpendEur / metric.shippedQty)
      ...
      const moroccoStock = metric.shippedQty - metric.soldQty;
  ...
  const enrichedOrders = [...orders]
    ...
    .map((order) => ({
      ...
      deliveryPriceMad: round(order.deliveryPriceMad ?? 0),
      itemsRevenueMad: round(
        order.itemsRevenueMad ?? sum(order.items ?? [], (item) => item.lineRevenueMad),
      ),
      paymentStatus: order.paymentStatus === "payee" ? "payee" : "non_payee",
      paidAt: order.paymentStatus === "payee" ? order.paidAt || order.orderedAt : null,
  ...
  const activity = [
    ...
    ...enrichedOrders.map((order) => ({
      ...
      amount: `${round(order.totalRevenueMad)} MAD`,
    })),
  ]
  ...
  const dashboard = {
    totals: {
      ...
      totalUnpaidOrders: enrichedOrders.filter((order) => order.paymentStatus !== "payee").length,
      outstandingRevenueMad: round(
        sum(
          enrichedOrders.filter((order) => order.paymentStatus !== "payee"),
          (order) => order.totalRevenueMad,
        ),
      ),
