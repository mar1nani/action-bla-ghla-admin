...
export function renderInvoiceHtml(order, settings, options = {}) {
  ...
  const advancePaidMad = Number(order.advancePaidMad ?? 0);
  const remainingToPayMad = Number(
    order.remainingToPayMad ?? Math.max((order.customerTotalMad ?? order.totalRevenueMad) - advancePaidMad, 0),
  );
  const balanceLabel =
  ...
  const advanceLine =
    advancePaidMad > 0
      ...
      <div class="totals-row">
        ...
        <strong>${escapeHtml(formatCurrency(advancePaidMad, "MAD"))}</strong>
      </div>
    `
      : "";
    ...
    <main class="page">
      ...
      <section class="summary-grid">
        ...
        <article class="summary-card ${order.paymentStatus === "payee" ? "" : "warning"}">
          ...
          <p>${escapeHtml(
            order.paymentStatus === "payee"
              ? "Cette commande est marquée comme réglée dans l'application."
              : order.paymentStatus === "avance"
                ? "Une avance a déjà été reçue. Encaisse seulement le solde à la livraison."
                : "Merci d'encaisser ce montant lors de la livraison.",
          )}</p>
        </article>
      </section>
      ...
      <section class="totals-card">
        ...
        <div class="totals-row">
          ...
          <strong>${escapeHtml(formatCurrency(remainingToPayMad, "MAD"))}</strong>
        </div>
      </section>
